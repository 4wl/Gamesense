...
void CVisuals::DrawArrow(CBaseEntity* pEnt)
{
	...
	if (!G::LocalPlayer || pEnt->IsDormant() || pEnt->IsWeapon() || !pEnt->IsPlayer() || pEnt->GetClientClass()->m_ClassID != (int)ClassID::CCSPlayer || pEnt == G::LocalPlayer || !pEnt->IsAlive() || pEnt->GetHealth() <= 0 || !Interfaces::Engine->IsInGame() || !Interfaces::Engine->IsConnected())
		return;

	int alpha = std::clamp(floor(sin(Interfaces::Globals->realtime * 4) * 127 + 128), 0.f, 0.5f * 255.f);
	Vector screenPos, client_viewangles2, client_viewangles;
	int screen_width = 0, screen_height = 0;
	Interfaces::Engine->GetScreenSize(screen_width, screen_height);
	//float radius = 50.f + ((screen_width / 4) * (c_config::get()->i["vis_outoffovarrows_dist"] / 100));
	float radius = 50.f + ((screen_width / 4.f)*(c_config::get()->i["vis_outoffovarrows_dist"] / 100.f));
	...
	if (G::LocalPlayer->GetEyePosition().y != NAN) {
		if (get_hitbox_pos(pEnt, HITBOX_HEAD).y != NAN){
		...
		auto line = pos - scr;
		Vector2D arrowBase = pos - (line * (c_config::get()->i["vis_outoffovarrows_size"] / (2 * (tanf(DEG2RAD(45)) / 2) * line.length())));
