...
void CEventLogger::Draw()
{
	...
	for ( auto &l : logs )
	{
		auto addy = ( c_config::get()->b["misc_showprefix"] ? Drawing->TextSize( "[gamesense] ", Drawing->fonts.esp_main ).right : 0 );


		/*if ( c_config::get()->b["misc_showprefix"] )
			Drawing->Text( 5, 5 + offset + l.yOffset, "[gamesense] ", Drawing->fonts.esp_main, c_config::get()->imcolor_to_ccolor(c_config::get()->c["misc_
			"]));*/
